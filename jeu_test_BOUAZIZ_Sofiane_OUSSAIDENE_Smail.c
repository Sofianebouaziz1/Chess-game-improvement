...
int estim7( struct config *conf, int mode, int alpha, int beta, int largeur, int numFctEst );
...
int estim7( struct config *conf, int mode, int alpha, int beta, int largeur, int numFctEst )
{
    ...
    if ( mode == MAX ) {
      // Generer les sucesseurs tactiques de cette configuration instable
	   ...
	   for ( i=0; i<n; i++ ) {
   	    	score2 = estim7( &T[i], MIN, score, beta, largeur, numFctEst);
      ...
      // Generer les sucesseurs tactiques de cette configuration instable
	   ...
	   for ( i=0; i<n; i++ ) {
        ...
        score2 = estim7( &T[i], MAX, alpha, score, largeur, numFctEst );
		if (score2 < score) score = score2;
		if (score <= alpha) {
			// Coupe Alpha
...
int minmax_ab( struct config *conf, int mode, int niv, int alpha, int beta, int largeur, int numFctEst )
{
    ...
    if (numFctEst == 6) {
   	...
   	if ( niv == 0 ){
        if (bol == 1) {
            return estim7(conf, mode, alpha, beta, largeur, numFctEst);
        }
        else return Est[numFctEst]( conf );
   	}
